...
(function(){
	...
	Signal.prototype.getDiscretValues = function(){
		this.discretValues = Signal.graficDataPoints(this.c);
	};

	Signal.prototype.parseDiscretValues = function(){
		var values = [];
		var length = this.discretValues.length;
		for(var i = 0; i < length; i++)
			values.push(this.discretValues[i].y);

		return values;
	};

	Signal.prototype.calcFFT = function(){
		var values = this.parseDiscretValues();
		var nDV = values.length; //number of discret values;
...
//	};
//
	Signal.graficDataPoints = function(data){
